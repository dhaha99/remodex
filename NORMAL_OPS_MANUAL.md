...
Gateway adapter가 정규화해 bridge로 넘기는 operator action은 interaction 계열 여덟 가지와 conversation text 계열 두 가지다.
...
- `conversation-bridge`
  - 바인딩된 채널의 plain text는 먼저 실제 bridge thread turn으로 들어간다.
  - bridge thread는 상태 질문에 자연어로 답하고, 작업 요청이면 handoff intent/reply를 구조화해 메인으로 넘긴다.
  - bridge thread의 지시문/행동 규약은 [DISCORD_BRIDGE_THREAD_GUIDEBOOK.md](/Users/mymac/my%20dev/remodex/DISCORD_BRIDGE_THREAD_GUIDEBOOK.md) 를 우선 본다.
...
- 바인딩된 채널에서는 plain text도 conversation surface로 받아야 한다.
- 바인딩된 채널의 plain text는 규칙 기반 템플릿 응답보다 실제 bridge thread turn을 우선 타는 편이 맞다.
- `지금 어디까지 했어?`, `현재 상태 알려줘` 같은 질문은 bridge thread가 shared memory snapshot과 최근 note를 읽고 자연어로 답하는 편이 맞다.
- 작업 요청은 bridge thread가 operator-facing 응답을 먼저 만들고, 그 뒤 handoff intent/reply를 structured inbox/dispatch 경로로 넘기는 편이 맞다.
- bridge thread 응답은 runtime validation을 통과해야 한다. 연결 질문인데 실제 메인 이름/짧은 ID/workspace를 빼먹거나, 내부 route/path/json 키를 노출하면 reject 후 재작성하는 편이 맞다.
- validation 재시도는 제한 횟수 안에서만 허용하고, 반복 실패 시에도 마지막 수단 safe fallback은 fact bundle 범위 안의 짧은 설명으로만 제한하는 편이 맞다.
- unbound 채널에서는 plain text를 무차별 수집하면 안 되고, bot mention이 있을 때만 도움말을 돌려주는 편이 맞다.
- human gate, processed completion, 중요 상태 변화는 `router/outbox/*`와 `processed/*` truth를 읽어 Discord 채널에 자동 알림을 보내는 편이 맞다.
- 자동 알림도 bridge thread가 note를 읽고 사람 말로 다시 정리하는 편이 맞고, raw receipt/outbox dump를 그대로 보내면 안 된다.
- Discord 자동 알림은 source of truth가 아니라 shared memory truth의 반영이어야 한다.
- Discord 앱에 Message Content intent가 안 켜져 있으면 adapter는 죽지 말고 `mention/slash-only` degraded mode로 다시 붙는 편이 맞다.
- degraded mode에서는 slash command, 버튼, mention 기반 도움말/대화는 유지하고, mention 없는 plain text bound-channel 대화만 제한되는 편이 맞다.
- degraded mode 응답에는 `대화 모드: mention_only` 안내와 함께 `@Remodex Pilot 지금 어디까지 했어?` 같은 실제 사용 예시를 같이 붙이는 편이 맞다.
- mention_only 상태에서 bound 채널의 plain text 질문/지시는 `@Remodex Pilot ...` 형태 멘션이 있을 때만 conversation surface로 태우는 편이 맞다.
...
- `router/discord_gateway_adapter_state.json`에 `conversation_mode = mention_only`, `bot_username = Remodex Pilot`가 찍혀 있으면 degraded fallback이 정상 적용된 상태로 봐야 한다.
