...
const probeDir =
  process.env.REMODEX_LAUNCHD_HUMAN_GATE_PROBE_DIR ??
  path.join(workspace, "verification", "launchd_human_gate_probe");
...
function collectBlockedReasons(toggleText, statusText, stopText) {
  const reasons = [];
  ...
  if (stopText?.includes("must_human_check: true")) reasons.push("must_human_check");
  if (stopText?.includes("pending_human_gate: MUST_HUMAN_CHECK")) reasons.push("pending_human_gate");
  return reasons;
}
...
try {
  ...
  const stopText = await readFileTrim(path.join(stateDir, "stop_conditions.md"));
  const wakeFileExists = await readFileTrim(input.wakeFilePath);
  const blockedReasons = collectBlockedReasons(toggleText, statusText, stopText);
  ...
  if (wakeFileExists !== null) {
    ...
    await client.initialize("remodex_launchd_human_gate_worker");
      ...
      input: [
        {
          type: "text",
          text:
            `Write the file ${input.wakeFilePath} with exact contents human-gate-cleared-ok. ` +
