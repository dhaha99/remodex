...
const probeRoot = path.join(verificationDir, "bridge_daemon_human_gate_notification_dedupe_probe");
...
const summaryPath = path.join(verificationDir, "bridge_daemon_human_gate_notification_dedupe_probe_summary.json");
const eventsLogPath = path.join(verificationDir, "bridge_daemon_human_gate_notification_dedupe_probe_events.jsonl");
const approvedPath = path.join(verificationDir, "bridge_daemon_human_gate_notification_dedupe_ok.txt");
const wrongPath = path.join(verificationDir, "bridge_daemon_human_gate_notification_dedupe_wrong.txt");
const daemonPort = 8793;
...
try {
  ...
  await client.initialize("bridge_daemon_human_gate_notification_dedupe_probe");
  ...
  const threadStart = await client.request("thread/start", {
    ...
    serviceName: "bridge_daemon_human_gate_notification_dedupe_probe",
  });
  ...
  const intentPayload = {
    id: "bridge-daemon-human-gate-dedupe-intent-001",
    type: 2,
    guild_id: "guild-1",
    ...
    data: {
      ...
      options: [
        { name: "project", value: "project-alpha" },
        {
          name: "request",
          value:
            `Create only ${approvedPath} with exact contents bridge-daemon-human-gate-dedupe-ok\\n. ` +
  ...
  const firstApproval = await client.waitForServerRequest(
    ["item/commandExecution/requestApproval", "item/fileChange/requestApproval"],
    (msg) => (msg.params?.threadId ?? null) === threadId,
    60_000,
  );
  summary.turnId = firstApproval.params?.turnId ?? null;
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    try {
      ...
      const nextApproval = await client.waitForServerRequest(
        ["item/commandExecution/requestApproval", "item/fileChange/requestApproval"],
        (msg) => (msg.params?.threadId ?? null) === threadId,
        60_000,
      );
      summary.approvalCount += 1;
      await client.respond(nextApproval.id, { decision: "accept" });
    }
  }
  if (!completed) {
    completed = await client.waitForNotification(
  ...
  await waitFor(async () => {
    const names = await fs.readdir(outboxDir).catch(() => []);
    return names.length > 0 ? names : null;
  }, 15_000);

  const outboxFiles = (await fs.readdir(outboxDir).catch(() => [])).sort();
  summary.outboxNotifications = await Promise.all(
    outboxFiles
      .filter((name) => name.includes("human_gate_notification"))
      .map(async (name) => ({
  ...
  summary.status =
    ...
    summary.approvalCount >= 2 &&
    summary.outboxNotifications.length === 1 &&
    summary.approvedContent === "bridge-daemon-human-gate-dedupe-ok\n" &&
