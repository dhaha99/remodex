...
const probeRoot = path.join(verificationDir, "discord_approval_human_gate_closure_probe");
...
const eventsLogPath = path.join(verificationDir, "discord_approval_human_gate_closure_probe_events.jsonl");
const summaryPath = path.join(verificationDir, "discord_approval_human_gate_closure_probe_summary.json");
const wsUrl = process.env.CODEX_APP_SERVER_WS_URL ?? "ws://127.0.0.1:4517";
const approvedFilePath = path.join(verificationDir, "discord_approval_closure_accepted.txt");
const wrongFilePath = path.join(verificationDir, "discord_approval_closure_wrong.txt");
...
try {
  ...
  await client.initialize("discord_approval_human_gate_closure_probe");
  ...
  const threadStart = await client.request("thread/start", {
    ...
    serviceName: "discord_approval_human_gate_closure_probe",
  });
    ...
    input: [
      {
        type: "text",
        text:
          `Create only ${approvedFilePath} with exact contents discord-approval-closure-ok\\n. ` +
  ...
  const approvePayload = {
    id: "discord-approve-closure-001",
    type: 2,
    guild_id: "guild-1",
    channel_id: "alpha-ops",
    timestamp: "2026-03-26T13:10:00+09:00",
    member: { user: { id: "ops-user-closure-1" }, roles: ["ops-admin", "operator"] },
    data: {
      name: "approve",
      options: [
  ...
  const candidatePath = path.join(humanGateDir, candidateFile);
  ...
  summary.status =
    ...
    summary.humanGateCandidate?.payload?.approval_source_ref === activeApprovalSourceRef &&
    summary.files.approved === "discord-approval-closure-ok\n" &&
    summary.files.wrong === null &&
    summary.files.inbox.length === 0 &&
    summary.files.dispatchQueue.length === 0 &&
    summary.files.quarantine.length === 0
      ? "PASS"
      : "FAIL";
} catch (error) {
  summary.finishedAt = new Date().toISOString();
...
if (summary.status !== "PASS") {
  throw new Error(summary.error ?? "discord approval human gate closure probe failed");
}
