...
const probeRoot = path.join(verificationDir, "foreground_dispatch_queue_drain_probe");
...
const summaryPath = path.join(verificationDir, "foreground_dispatch_queue_drain_probe_summary.json");
const eventsLogPath = path.join(verificationDir, "foreground_dispatch_queue_drain_probe_events.jsonl");
const optionAPath = path.join(verificationDir, "foreground_drain_option_a.txt");
const optionBPath = path.join(verificationDir, "foreground_drain_option_b.txt");
...
let client = null;
let foregroundClient = null;

try {
  await fs.rm(probeRoot, { recursive: true, force: true });
  await fs.writeFile(eventsLogPath, "");
  for (const filePath of [optionAPath, optionBPath]) {
  ...
  await client.initialize("remodex_foreground_dispatch_queue_drain_probe");

  const threadStart = await client.request("thread/start", {
    cwd: workspace,
    approvalPolicy: "never",
    sandbox: "workspace-write",
    serviceName: "remodex_foreground_dispatch_queue_drain_probe",
  });
    ...
    input: [
      {
        type: "text",
        text:
          `Before making any file changes, ask exactly one clarifying question about ${optionAPath} and ${optionBPath}. ` +
          `Then stop and wait for my next turn. Do not create either file yet.`,
      },
    ],
  });
  ...
  const inboxFileName = "2026-03-26T09-30-00+09-00_operator_answer.json";
  await writeJson(path.join(inboxDir, inboxFileName), {
    workspace_key: "remodex",
    project_key: "project-alpha",
    correlation_key: "foreground-dispatch-drain-001",
    operator_answer:
      `Answer to your last question: create only ${optionBPath} with exact contents foreground-drain-ok\\n. ` +
      `Do not create ${optionAPath}.`,
  });

  summary.bridgeDecision = await bridgeDeferIfForeground();

  const threadAfterDefer = await client.request("thread/read", {
    threadId,
    includeTurns: true,
  });
  summary.threadAfterDefer = {
    turnCount: (threadAfterDefer?.thread?.turns ?? []).length,
    status: threadAfterDefer?.thread?.status ?? null,
  };

  foregroundClient = new JsonRpcWsClient(wsUrl, eventsLogPath);
  await foregroundClient.connect();
  await foregroundClient.initialize("remodex_foreground_dispatch_queue_drain_runner");

  summary.drainDecision = await foregroundDrainQueue(foregroundClient);

  summary.finalFiles = {
    optionA: await readIfExists(optionAPath),
    optionB: await waitForFile(optionBPath, 15_000),
  };

  const combined = new Map();
  for (const currentClient of [client, foregroundClient]) {
  ...
  summary.status =
    ...
    summary.threadAfterDefer?.turnCount === 1 &&
    summary.drainDecision?.decision === "drained" &&
    summary.finalFiles.optionA === null &&
    summary.finalFiles.optionB === "foreground-drain-ok\n"
      ? "PASS"
      : "FAIL";
} catch (error) {
  summary.status = "FAIL";
  ...
  foregroundClient?.clearAllWaiters();
  client?.close();
  foregroundClient?.close();
}
