...
const probeDir = path.join(verificationDir, "launchd_conditional_delivery_probe");
...
const plistPath = path.join(verificationDir, "com.remodex.launchd-conditional-delivery.plist");
const summaryPath = path.join(verificationDir, "launchd_conditional_delivery_probe_summary.json");
...
const wakeFilePath = path.join(probeDir, "from_conditional_launchd_wake.txt");
const workerScriptPath = path.join(workspace, "scripts", "launchd_conditional_delivery_worker.mjs");
const nodePath = "/opt/homebrew/bin/node";
const label = "com.remodex.launchd-conditional-delivery";
...
async function writeToggleAndStatus({ backgroundEnabled, foregroundActive, statusType }) {
  ...
  await writeText(path.join(stateDir, "coordinator_status.md"), `type: ${statusType}\n`);
}
...
try {
  ...
  const wakeFileBlocked = await readTextIfExists(wakeFilePath);
  summary.blockedCase = {
    run: blockedRun,
    wakeFileExists: wakeFileBlocked !== null,
  };
  await execFileAsync("launchctl", ["bootout", `${guiDomain}/${label}`]);
