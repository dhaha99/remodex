...
const probeDir = path.join(verificationDir, "launchd_worker_persistence_probe");
...
const plistPath = path.join(verificationDir, "com.remodex.launchd-worker-persistence.plist");
const summaryPath = path.join(verificationDir, "launchd_worker_persistence_probe_summary.json");
...
const eventsLogPath = path.join(verificationDir, "launchd_worker_persistence_probe_events.jsonl");
const targetPath = path.join(verificationDir, "launchd_worker_persistence_target.txt");
const wrongPath = path.join(verificationDir, "launchd_worker_persistence_wrong.txt");
const wsUrl = process.env.CODEX_APP_SERVER_WS_URL ?? "ws://127.0.0.1:4517";
const nodePath = "/opt/homebrew/bin/node";
const label = "com.remodex.launchd-worker-persistence";
...
async function writeToggleAndStatus() {
  ...
  await fs.writeFile(path.join(stateDir, "coordinator_status.md"), "type: checkpoint_open\n");
}
...
<dict>
  ...
  <dict>
    <key>REMODEX_LAUNCHD_PROBE_DIR</key>
...
try {
  ...
  await client.initialize("probe_launchd_worker_persistence");

  const threadStart = await client.request("thread/start", {
    cwd: workspace,
    approvalPolicy: "never",
    sandbox: "workspace-write",
    serviceName: "probe_launchd_worker_persistence",
  });
  ...
  const payload = {
    id: "discord-persistence-001",
    type: 2,
    guild_id: "guild-1",
    channel_id: "alpha-ops",
    timestamp: "2026-03-26T13:00:00+09:00",
    member: { user: { id: "ops-user-persist" }, roles: ["ops-admin", "operator"] },
    data: {
      name: "intent",
      options: [
        { name: "project", value: "project-alpha" },
        {
          name: "request",
          value:
            `Answer to your last question: create only ${targetPath} with exact contents launchd-persistence-ok\\n. ` +
  ...
  const inflightClaim = await waitFor(async () => readJsonIfExists(inflightPath), 20_000);
  summary.interruptedWake = { inflightClaim };
  if (!inflightClaim) {
    throw new Error("launchd worker did not record inflight claim");
  }
  await execFileAsync("launchctl", ["bootout", `${guiDomain}/${label}`]);

  const targetText = await waitFor(async () => readTextIfExists(targetPath), 120_000);
  if (targetText !== "launchd-persistence-ok\n") {
    throw new Error("background turn did not finish after worker interruption");
  }

  const beforeRecoveryTurnCount = await readTurnCount(client, threadId);
  await fs.rm(lastRunPath, { force: true });
  await execFileAsync("launchctl", ["bootstrap", guiDomain, plistPath]);
  const recoveryRun = await waitFor(async () => {
    const run = await readJsonIfExists(lastRunPath);
    return run?.decision === "completed_inflight" ? run : null;
  }, 30_000);
  const afterRecoveryTurnCount = await readTurnCount(client, threadId);
  await execFileAsync("launchctl", ["bootout", `${guiDomain}/${label}`]);

  summary.recoveryPhase = {
    run: recoveryRun,
    beforeRecoveryTurnCount,
    afterRecoveryTurnCount,
    processedFiles: (await fs.readdir(processedDir).catch(() => [])).sort(),
  ...
  summary.status =
    ...
    summary.interruptedWake?.inflightClaim?.turnId &&
    summary.recoveryPhase?.run?.decision === "completed_inflight" &&
    summary.recoveryPhase.beforeRecoveryTurnCount === summary.recoveryPhase.afterRecoveryTurnCount &&
    summary.recoveryPhase.processedFiles.length >= 1 &&
    summary.finalFiles.target === "launchd-persistence-ok\n" &&
...
if (summary.status !== "PASS") {
  throw new Error(summary.error ?? "launchd worker persistence probe failed");
}
