...
const probeRoot = path.join(verificationDir, "processed_correlation_dedupe_probe");
...
const summaryPath = path.join(verificationDir, "processed_correlation_dedupe_probe_summary.json");
const eventsLogPath = path.join(verificationDir, "processed_correlation_dedupe_probe_events.jsonl");
const targetPath = path.join(verificationDir, "processed_dedupe_target.txt");
const wrongPath = path.join(verificationDir, "processed_dedupe_wrong.txt");
...
async function bridgeDeferIfForeground() {
  ...
  if (toggle.foreground_session_active || !toggle.background_trigger_enabled || !["idle", "checkpoint_open"].includes(status.type)) {
    ...
    return { decision: "deferred", ticketPath, inboxFile, correlationKey: event.correlation_key };
  }
...
async function foregroundDrain(client) {
  ...
  return { decision: "drained", queueFile, correlationKey: ticket.correlation_key, ...turn };
}
...
async function safeRecoveryReplay(client) {
  ...
  for (const processedFile of processedFiles) {
    const processed = JSON.parse(await fs.readFile(path.join(processedDir, processedFile), "utf8"));
    if (processed.correlation_key && processed.correlation_key === event.correlation_key) {
      return {
  ...
  return { decision: "unexpected_replay", inboxFile, correlationKey: event.correlation_key, ...turn };
}
...
const summary = {
  ...
  bridgeDecision: null,
  firstDrain: null,
  safeReplayDecision: null,
  finalFiles: null,
};

let seedClient = null;
let drainClient = null;
let safeReplayClient = null;
...
try {
  ...
  await seedClient.initialize("remodex_processed_dedupe_seed");

  const threadStart = await seedClient.request("thread/start", {
    cwd: workspace,
    approvalPolicy: "never",
    sandbox: "workspace-write",
    serviceName: "remodex_processed_dedupe_seed",
  });
  ...
  await writeJson(path.join(inboxDir, "2026-03-26T09-50-00+09-00_operator_answer.json"), {
    workspace_key: "remodex",
    project_key: "project-alpha",
    correlation_key: "processed-correlation-dedupe-001",
    operator_answer:
      `Answer to your last question: append exactly one new line processed-correlation-evidence to ${targetPath}. ` +
      `Do not create or modify ${wrongPath}.`,
  });
  ...
  await drainClient.initialize("remodex_processed_dedupe_drain");
  summary.firstDrain = await foregroundDrain(drainClient);

  safeReplayClient = new JsonRpcWsClient(wsUrl, eventsLogPath);
  await safeReplayClient.connect();
  await safeReplayClient.initialize("remodex_processed_dedupe_router");
  summary.safeReplayDecision = await safeRecoveryReplay(safeReplayClient);
  ...
  summary.targetLineCount = finalTarget?.split("\n").filter(Boolean).length ?? 0;

  const combined = new Map();
  for (const currentClient of [seedClient, drainClient, safeReplayClient]) {
  ...
  summary.status =
    ...
    summary.safeReplayDecision?.decision === "skipped_duplicate_correlation" &&
    summary.finalFiles.target === "processed-correlation-evidence\n" &&
    summary.finalFiles.wrong === null &&
    summary.targetLineCount === 1
      ? "PASS"
      : "FAIL";
} catch (error) {
  summary.status = "FAIL";
  ...
  safeReplayClient?.clearAllWaiters();
  seedClient?.close();
  drainClient?.close();
  safeReplayClient?.close();
}
